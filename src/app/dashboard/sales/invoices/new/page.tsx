...
export default function NewInvoicePage() {
  ...
  // Auto-set due date when customer changes
  useEffect(() => {
    const customer = customers.find(c => c.id === customerId);
    if (customer && date) {
      const d = new Date(date);
      d.setDate(d.getDate() + (customer.paymentTermDays || 30));
  ...
  const submit = async (sendInvoice: boolean) => {
    ...
    try {
      const res = await fetch("/api/v1/sales/invoices", {
        ...
        body: JSON.stringify({
          customerId, date, dueDate, reference, notes, terms,
      ...
      setSuccess(`Invoice ${data.invoice.invoiceNumber} ${sendInvoice ? "sent" : "saved"} successfully`);
      setTimeout(() => router.push("/dashboard/sales/invoices"), 1200);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500">Create a customer invoice</p>
          </div>
        </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1">
              ...
              {selectedCustomer && (
                <p className="text-xs text-slate-500">Payment terms: {selectedCustomer.paymentTermDays} days</p>
              )}
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700">Reference</label>
              <Input placeholder="e.g. PO-001, Contract ref" value={reference} onChange={e => setReference(e.target.value)} />
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700">Invoice Date <span className="text-red-500">*</span></label>
      ...
      <div className="flex justify-end gap-3 pb-6">
        <Link href="/dashboard/sales/invoices"><Button variant="ghost">Cancel</Button></Link>
        <Button variant="outline" onClick={() => submit(false)} disabled={loading}>
          <Save className="mr-2 h-4 w-4" />Save Draft
        </Button>
        <Button onClick={() => submit(true)} disabled={loading}>
