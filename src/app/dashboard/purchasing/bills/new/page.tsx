...
import { ArrowLeft, Plus, Trash2, Save, CheckCircle2, AlertCircle, Send } from "lucide-react";

interface Supplier { id: string; code: string; name: string; paymentTermDays: number; }
...
export default function NewBillPage() {
  ...
  useEffect(() => {
    fetch("/api/v1/purchasing/suppliers").then(r => r.json()).then(d => setSuppliers(d.suppliers || []));
    fetch("/api/v1/accounting/accounts").then(r => r.json()).then(d =>
      setAccounts((d.accounts || []).filter((a: Account) => a.accountType === "expense" || a.accountType === "asset"))
    );
  }, []);
  ...
  const submit = async (approve: boolean) => {
    ...
    if (!date || !dueDate) { setError("Date and due date are required"); return; }
    ...
    try {
      const res = await fetch("/api/v1/purchasing/bills", {
        ...
        body: JSON.stringify({
          supplierId, supplierInvoiceNumber, date, dueDate, notes,
      ...
      setSuccess(`Bill ${data.bill.billNumber} ${approve ? "approved and posted" : "saved as draft"}`);
      setTimeout(() => router.push("/dashboard/purchasing/bills"), 1200);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500">Record a supplier bill</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => submit(true)} disabled={loading}>
            <Send className="mr-2 h-4 w-4" />{loading ? "Saving..." : "Approve & Post"}
          </Button>
        </div>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1">
              ...
              {selectedSupplier && (
                <p className="text-xs text-slate-500">Payment terms: {selectedSupplier.paymentTermDays} days</p>
              )}
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700">Supplier Invoice Number</label>
              <Input placeholder="Supplier's own invoice number" value={supplierInvoiceNumber} onChange={e => setSupplierInvoiceNumber(e.target.value)} />
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-700">Bill Date <span className="text-red-500">*</span></label>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>What was purchased and which expense account to charge</CardDescription>
            </div>
        ...
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="pb-2 text-left font-medium text-slate-500 w-[25%]">Expense Account</th>
          ...
          <div className="mt-6 flex justify-end">
            <div className="w-72 space-y-2">
              ...
              <div className="flex justify-between text-sm text-slate-600">
                <span>GST (Input Tax Credit)</span><span className="font-mono">{taxTotal.toFixed(2)}</span>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <textarea rows={3} placeholder="Internal notes about this bill..."
      ...
      <div className="flex justify-end gap-3 pb-6">
        <Link href="/dashboard/purchasing/bills"><Button variant="ghost">Cancel</Button></Link>
        <Button variant="outline" onClick={() => submit(false)} disabled={loading}>
          <Save className="mr-2 h-4 w-4" />Save Draft
        </Button>
        <Button onClick={() => submit(true)} disabled={loading}>
