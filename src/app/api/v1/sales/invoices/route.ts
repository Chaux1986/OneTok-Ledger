...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create journal entry for sent invoices
    if (data.status === "sent") {
      // Get AR and GST accounts
      ...
      if (arAccount && gstAccount) {
        ...
        // Journal lines: DR Trade Debtors, CR Revenue accounts, CR GST
        const jLines = [];

        // DR Trade Debtors (full invoice amount)
        jLines.push({
