...
export function InvoicePayButton({ invoiceId, invoiceNumber, amountDue, currency }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/sales/invoices/${invoiceId}/payment`, {
        method: "POST",
      ...
      setSuccess(`Recorded — invoice is now ${data.invoice.status}`);
      setTimeout(() => {
        setOpen(false);
        router.refresh();
      }, 900);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !loading && setOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-semibold text-slate-900">Receive Payment</h3>
                <p className="text-xs text-slate-500">{invoiceNumber} · Due {currency} {amountDue.toFixed(2)}</p>
              </div>
